...
const handleRemoveFromCart = async (carId) => {
  try {
    const token = localStorage.getItem('token');
    const response = await axios.delete(`http://localhost:5000/api/cart/${carId}`, {
      headers: {
  ...
  {cartItems.map((item) => (
    <Card key={item.carId} className="shadow-lg rounded-lg p-4">
      <Card.Img
        variant="top"
        src={`http://localhost:5000/${item.carImage}`}
